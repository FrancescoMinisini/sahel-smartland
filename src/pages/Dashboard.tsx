...
import { Calendar, FileText, Download, BarChart2, ArrowRight, Leaf, Map, CloudRain, Users, Clock, Filter, Layers, ZoomIn, HelpCircle, Info, Sprout, Trees, Expand } from 'lucide-react';
import { Button } from "@/components/ui/button";
import { landCoverClasses, landCoverColors, regionalPrecipitationColors, getAccuratePrecipitationData, getPrecipitationTimeSeriesData, getLandCoverTimeSeriesData, loadPrecipitationByRegion, getVegetationTimeSeriesData } from '@/lib/geospatialUtils';
...
const Dashboard = () => {
  ...
  const [vegetationTimeSeriesData, setVegetationTimeSeriesData] = useState<Array<{
    year: number;
    [key: string]: number;
  }>>([]);
  const [regionalPrecipitationData, setRegionalPrecipitationData] = useState<Array<{
    year: number;
    Overall: number;
    South: number;
    Center: number;
  ...
  const handleYearChange = (year: number) => {
    ...
    if (year !== selectedYear) {
      ...
      setPreviousVegetationStats({
        ...vegetationStats
      });
      setPreviousPopulationStats({
        ...populationStats
      });
      setIsLoading(true);
  ...
  const chartData = Object.entries(landCoverStats).filter(([key]) => key !== '0').map(([key, value]) => {
  ...
  const vegetationChartData = (() => {
    const currentYear = vegetationTimeSeriesData.find(d => d.year === selectedYear) || vegetationTimeSeriesData[vegetationTimeSeriesData.length - 1] || {
      year: selectedYear,
      Forest: 1200,
      Grassland: 800,
      Cropland: 900,
  ...
  const keyStats = [{
    ...
    icon: <Leaf size={20} />,
    trend: {
      value: selectedYear > 2020 ? 15 : selectedYear > 2015 ? 12 : 8,
      isPositive: false
    },
    analyticsData: [{
      year: 2010,
    ...
    trend: {
      value: selectedYear > 2020 ? 3.5 : selectedYear > 2015 ? 2.1 : 1.2,
      isPositive: false
    },
    analyticsData: [{
      year: 2010,
      ...
      <main className="flex-1 pt-24 pb-12">
        <div className="container mx-auto px-6">
          ...
          <div className="bg-white dark:bg-muted rounded-xl shadow-sm border border-border/40 p-6 mb-8">
            ...
            <YearSlider minYear={2010} maxYear={2023} initialValue={selectedYear} onChange={handleYearChange} className="max-w-3xl mx-auto" autoPlay={false} autoPlayInterval={1500} />
          </div>
          ...
          <div className="bg-white dark:bg-muted rounded-xl shadow-sm border border-border/40 overflow-hidden mb-8">
            <div className="flex flex-wrap border-b border-border/40">
              {dataTabs.map(tab => <button key={tab.id} onClick={() => handleTabChange(tab.id)} className={`flex items-center px-6 py-4 text-sm font-medium transition-colors ${activeTab === tab.id ? "text-sahel-green border-b-2 border-sahel-green" : "text-muted-foreground hover:text-foreground hover:bg-muted/40"}`}>
                  {tab.icon}
              ...
              <div className="ml-auto flex items-center px-4">
                ...
                <Link to={`/temporal-analysis?tab=${activeTab}&year=${selectedYear}`} className="p-2 rounded-md hover:bg-muted transition-colors ml-1">
            ...
            <div className="p-6">
              {isLoading ? <div className="h-[400px] flex items-center justify-center">
                  ...
                  {activeTab === 'landCover' && <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      <div className="lg:col-span-2 space-y-6">
                        <div className="bg-white dark:bg-muted rounded-lg border border-border/40 p-6">
                          ...
                          <ChartCarousel data={chartData} timeSeriesData={timeSeriesData} dataType="landCover" />
                        </div>
                      </div>

                      <div className="lg:col-span-1 h-full">
                        <div className="h-[400px]">
                          <MapVisualization className="w-full h-full" year={selectedYear} expandedView={true} onStatsChange={handleStatsChange} dataType="landCover" />
                        </div>
                        <div className="text-center mt-3">
                          <a href="/sahel_map.html" target="_blank" rel="noopener noreferrer" className="text-sm text-sahel-blue flex items-center justify-center hover:underline">
                  ...
                  {activeTab === 'vegetation' && <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      <div className="lg:col-span-2 space-y-6">
                        <div className="bg-white dark:bg-muted rounded-lg border border-border/40 p-6">
                          <h3 className="text-lg font-medium mb-4">Vegetation Productivity ({selectedYear})</h3>
                          <ChartCarousel data={vegetationChartData} timeSeriesData={vegetationTimeSeriesData} dataType="vegetation" />
                        </div>
                      </div>

                      <div className="lg:col-span-1 h-full">
                        <div className="h-[400px]">
                          <MapVisualization className="w-full h-full" year={selectedYear} expandedView={true} onStatsChange={handleStatsChange} dataType="vegetation" />
                        </div>
                        <div className="text-center mt-3">
                          <a href="/sahel_map.html" target="_blank" rel="noopener noreferrer" className="text-sm text-sahel-blue flex items-center justify-center hover:underline">
                            <ZoomIn size={14} className="mr-1" /> 
                  ...
                  {activeTab === 'precipitation' && <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      <div className="lg:col-span-2 space-y-6">
                        <div className="bg-white dark:bg-muted rounded-lg border border-border/40 p-6">
                          <h3 className="text-lg font-medium mb-4">Precipitation by Region ({selectedYear})</h3>
                          <ChartCarousel data={regionalPrecipChartData} timeSeriesData={regionalPrecipitationData} dataType="precipitation" />
                        </div>
                      </div>

                      <div className="lg:col-span-1 h-full">
                        <div className="h-[400px]">
                          <MapVisualization className="w-full h-full" year={selectedYear} expandedView={true} onStatsChange={handleStatsChange} dataType="precipitation" />
                        </div>
                        <div className="text-center mt-3">
                          <a href="/sahel_map.html" target="_blank" rel="noopener noreferrer" className="text-sm text-sahel-blue flex items-center justify-center hover:underline">
                            <ZoomIn size={14} className="mr-1" /> 
                  ...
                  {activeTab === 'population' && <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      <div className="lg:col-span-2 space-y-6">
                        <div className="bg-white dark:bg-muted rounded-lg border border-border/40 p-6">
                          <h3 className="text-lg font-medium mb-4">Population Distribution ({selectedYear})</h3>
                          <ChartCarousel data={populationChartData} timeSeriesData={[{
                      year: 2010,
                      Urban: 1900000,
                      Rural: 3500000,
                      Nomadic: 850000,
                      ...
                      <div className="lg:col-span-1 h-full">
                        <div className="h-[400px]">
                          <MapVisualization className="w-full h-full" year={selectedYear} expandedView={true} onStatsChange={handleStatsChange} dataType="population" />
                        </div>
                        <div className="text-center mt-3">
                          <a href="/sahel_map.html" target="_blank" rel="noopener noreferrer" className="text-sm text-sahel-blue flex items-center justify-center hover:underline">
                            <ZoomIn size={14} className="mr-1" /> 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            {keyStats.map((stat, index) => <DataCard key={index} title={stat.title} value={stat.value} description={stat.description} icon={stat.icon} trend={stat.trend} analyticsData={stat.analyticsData} correlations={stat.correlations} year={selectedYear} />)}
          </div>
